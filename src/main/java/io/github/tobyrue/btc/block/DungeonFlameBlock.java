...
public class DungeonFlameBlock extends Block {
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        for(i = 0; i < 3; ++i) {
            ...
            if (this.config.getParticle(state.get(Properties.POWERED)) != null) {
                world.addParticle(config.getParticle(state.get(Properties.POWERED)), d2, e2, f2, 0.0, 0.0, 0.0);
            }
            if (this.config.isLit(state.get(Properties.POWERED))) {
    ...
    public static int getLuminance(BlockState currentBlockState) {
        return currentBlockState.get(Properties.POWERED) ? 15 : 0;
    }

    @Override
    protected void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, BlockPos sourcePos, boolean notify) {
        world.setBlockState(pos, state.with(Properties.POWERED,
    ...
    protected void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        if (config.isLit(state.get(Properties.POWERED))) {
            ...
            entity.damage(world.getDamageSources().inFire(), config.getDamage(state.get(Properties.POWERED)));
        }
