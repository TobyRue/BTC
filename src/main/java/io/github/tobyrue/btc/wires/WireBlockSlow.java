...
//        REDSTONE("redstone", 0xFF745B /* Coral */,args -> false);
    ...
    public WireBlockSlow(Settings settings, boolean immutable) {
        super(settings);
        IMMUTABLE = immutable;
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        ...
        if (!world.isClient && state.getBlock() instanceof WireBlockSlow) {
            if (state.getBlock() != newState.getBlock()) {
                for (Direction direction : Direction.values()) {
                    BlockPos neighborPos = pos.offset(direction);
                    BlockState neighborState = world.getBlockState(neighborPos);
                    var property = state.get(WireBlockSlow.CONNECTION_TO_DIRECTION.get().inverse().get(direction.getOpposite()));

                    if (neighborState.getBlock() instanceof IDungeonWireAction action && property == WireBlockSlow.ConnectionType.OUTPUT) {
        ...
        return state.get(OPERATOR).apply(
                CONNECTION_TO_DIRECTION.get().entrySet().stream()
                        ...
                        .map(entry -> {
                            ...
                            return (offsetState.contains(WireBlockSlow.POWERED)
