...
public class WaterBlastItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            WaterBlastEntity waterBlast = new WaterBlastEntity(user, world, user.getX(), user.getY(), user.getZ());
            waterBlast.setVelocity(user, user.getPitch(), user.getYaw(), 0.0f, 1.5f, 0f);
